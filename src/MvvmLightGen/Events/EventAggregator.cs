    ...
    public class EventAggregator : IEventAggregator
    {
        ...
        public void RegisterSubscriber<TSubscriber>(TSubscriber subscriber)
        {
            ...
            var subscriberInterfaces = typeof(TSubscriber).GetInterfaces()
                .Where(t => t.IsGenericType && t.FullName?.StartsWith("MvvmLightGen.Events.IEventSubscriber") == true).ToList();
        ...
        /// <summary>
        /// Unregisters an MvvmLightGen.Events.IEventSubscriber, so that it won't receive events anymore from the IEventAggregator instance.
