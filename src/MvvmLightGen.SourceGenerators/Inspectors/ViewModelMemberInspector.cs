...
namespace MvvmLightGen.Inspectors
{
    internal static class ViewModelMemberInspector
    {
        ...
        private static void FindPropertyInvalidations(Dictionary<string, List<string>> propertyInvalidationsByPropertyName, IPropertySymbol propertySymbol)
        {
            ...
            var invalidateAttributeDatas = attributeDatas.Where(x => x.AttributeClass?.ToDisplayString() == "MvvmLightGen.PropertyInvalidateAttribute").ToList();
        ...
        private static void FindPropertiesToGenerate(IFieldSymbol fieldSymbol, List<PropertyToGenerate> propertiesToGenerate)
        {
            ...
            var propertyAttributeData = attributeDatas.FirstOrDefault(x => x.AttributeClass?.ToDisplayString() == "MvvmLightGen.PropertyAttribute");
            ...
            if (propertyAttributeData is not null)
            {
                ...
                var propertyPublishEventAttributes = attributeDatas.Where(x => x.AttributeClass?.ToDisplayString() == "MvvmLightGen.PropertyPublishEventAttribute").ToList();
                var propertyCallMethodAttributes = attributeDatas.Where(x => x.AttributeClass?.ToDisplayString() == "MvvmLightGen.PropertyCallMethodAttribute").ToList();
            ...
            System.Collections.Immutable.ImmutableArray<ISymbol> viewModelMembers)
        {
            ...
            var commandAttributeData = methodAttributes.FirstOrDefault(x => x.AttributeClass?.ToDisplayString() == "MvvmLightGen.CommandAttribute");

            var invalidateAttributeDatas = methodAttributes.Where(x => x.AttributeClass?.ToDisplayString() == "MvvmLightGen.CommandInvalidateAttribute").ToList();
